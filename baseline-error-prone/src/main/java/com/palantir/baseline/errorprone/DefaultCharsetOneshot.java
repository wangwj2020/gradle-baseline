...
public class DefaultCharsetOneshot extends BugChecker
    implements MethodInvocationTreeMatcher, NewClassTreeMatcher {

  enum CharsetFix {
    UTF_8_FIX("StandardCharsets.UTF_8") {
      @Override
      void addImport(Builder fix, VisitorState state) {
        fix.addImport("java.nio.charset.StandardCharsets");
      }
    },
    ...
    abstract void addImport(Builder fix, VisitorState state);
  }
  ...
  private static Builder byteStringFix(
      MethodInvocationTree tree,
      ExpressionTree parent,
      VisitorState state,
      String prefix,
  ...
  private void variableTypeFix(
      Builder fix, VisitorState state, Class<?> original, Class<?> replacement) {
    Tree parent = state.getPath().getParentPath().getLeaf();
    Symbol sym;
  ...
  private String toAppendMode(Builder fix, Tree appendArg, VisitorState state) {
  ...
  private Object toFile(VisitorState state, Tree fileArg, Builder fix) {
  ...
  private String toPath(VisitorState state, Tree fileArg, Builder fix) {
    Type type = ASTHelpers.getType(fileArg);
    if (ASTHelpers.isSubtype(type, state.getSymtab().stringType, state)) {
